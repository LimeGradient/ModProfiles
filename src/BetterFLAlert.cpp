...
bool BetterFLAlert::init(float w, float h, const char* spr, const char* title) {
    ...
    if (strlen(title) > 0) {
        ...
        titleLabel->limitLabelWidth(this->m_layerSize.width * 4, .8f, .2f);
        titleLabel->setPosition(
            winSize.width / 2,
            winSize.height / 2 + this->m_layerSize.height / 2 - 25
        );
    ...
    auto closeBtn = CCMenuItemSpriteExtra::create(
        closeSpr,
        this,
        (cocos2d::SEL_MenuHandler)&BetterFLAlert::onClose
    );
...
void BetterFLAlert::keyDown(cocos2d::enumKeyCodes key) {
...
void BetterFLAlert::onClose(cocos2d::CCObject* pSender) {
